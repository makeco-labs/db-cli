 ...
 */
export async function seedSingleStoreDatabase(seedPath: string): Promise<SeedResult> {
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error during SingleStore seed',
      timestamp,
    };
  }
}