...
export async function listMysqlTables(
  ...
  try {
    ...
    const schemaRows = Array.isArray(schemaResult) ? schemaResult : schemaResult.rows || [schemaResult];
    const schemas = (schemaRows as Array<{ schema_name: string }>).map(row => row.schema_name);
    ...
    for (const schemaName of schemas) {
      ...
      const tableRows = Array.isArray(tableResult) ? tableResult : tableResult.rows || [tableResult];
      ...
      if (tableNames.length > 0) {
        ...
        for (const tableName of tableNames) {
          ...
          if (includeRowCounts) {
            tableInfo.rowCount = await getTableRowCount(connection, schemaName, tableName);
          }
