 ...
 */
export async function getTables(connection: PostgresConnection): Promise<string[]> {
  const statement = sql`
    SELECT table_name
 ...
 */
export async function getSchemas(connection: PostgresConnection): Promise<string[]> {
  const statement = sql`
    SELECT schema_name
  ...
  return (schemas as Array<{ schema_name: string }>)
    ...
    .filter(schema => !schemaAllowlist.includes(schema) && schema !== 'public');
}

/**
 * Gets all user tables in non-public schemas
 */
export async function getTablesInSchemas(connection: PostgresConnection, schemas: string[]): Promise<string[]> {
  ...
  return (tables as Array<{ full_table_name: string }>)
    .map((row) => row.full_table_name);
}